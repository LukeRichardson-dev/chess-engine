...
const SIDE_BLACK: u8 = 1 << 6; // WTF
const SIDE_WHITE: u8 = 1 << 7; // Why are there four possible colours?
...
pub fn encode_as(bm: &BoardMap, side: Side) -> Vec<u8> {
    let flip_mask = if let Side::White = side {
        SIDE_BLACK
    } else {
        0
    };

    bm.arr.iter().map(|x| x ^ flip_mask).collect()
}
